...
public class CategoryExtremeRecipe implements IRecipeCategory<RecipeExtremeCrafting> {
	...
	public CategoryExtremeRecipe(IGuiHelper guiHelper) {
		...
		this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(AvaritiaBlocks.EXTREME_CRAFTING_TABLE_ITEM.get()));
	}
	...
	public void setRecipe(IRecipeLayoutBuilder recipeLayoutBuilder, RecipeExtremeCrafting recipe, IFocusGroup group) {
		...
		recipeLayoutBuilder.addSlot(RecipeIngredientRole.OUTPUT, 168, 74).addIngredients(VanillaTypes.ITEM_STACK, List.of(resultItem));
		...
		if (recipe instanceof RecipeExtremeShaped) {
			...
			for (int y = 0; y < 9; y++) {
				for (int x = 0; x < 9; x++) {
					...
					inputSlots.get(slotIndex).addIngredients(VanillaTypes.ITEM_STACK, inputs.get(inputIndex));
				}
			}
		} else {
			recipeLayoutBuilder.setShapeless();
			for (int i = 0; i < inputs.size(); i++) {
				inputSlots.get(i).addIngredients(VanillaTypes.ITEM_STACK, inputs.get(i));
			}
		}
	}
