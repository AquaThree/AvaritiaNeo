...
public class TransferHandlerExtremeRecipe {
	...
	public static <C extends AbstractContainerMenu, D extends Display> SimpleTransferHandler create(
			Class<? extends C> containerClass, CategoryIdentifier<D> categoryIdentifier, IntRange inputSlots) {
		return new SimpleTransferHandler() {
			...
			public Iterable<SlotAccessor> getInputSlots(Context context) {
				...
				return IntStream.range(inputSlots.min(), inputSlots.maxExclusive())
						.mapToObj(id -> SlotAccessor.fromSlot(context.getMenu().getSlot(id))).toList();
			}
			...
			@Override
			public void renderMissingInput(Context context, List<InputIngredient<ItemStack>> inputs,
					List<InputIngredient<ItemStack>> missing, IntSet missingIndices, GuiGraphics graphics, int mouseX,
					int mouseY, float partialTicks, List<Widget> widgets, Rectangle rectangle) {
				DisplayExtremeRecipe display = (DisplayExtremeRecipe) context.getDisplay();
				RecipeExtremeCrafting recipe = display.getRecipe();
				List<EntryIngredient> ingredients = display.getInputEntries();
