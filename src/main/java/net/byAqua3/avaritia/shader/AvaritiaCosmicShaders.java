	...
	public static void onTextureAtlasStitched(TextureAtlasStitchedEvent event) {
		if (event.getAtlas().location().equals(AvaritiaAtlas.BLOCK_ATLAS)) {
			for (int i = 0; i < COSMIC_SPRITES.length; i++) {
				...
				AvaritiaCosmicShaders.COSMIC_UVS[i * 4 + 0] = COSMIC_SPRITES[i].getU0();
				AvaritiaCosmicShaders.COSMIC_UVS[i * 4 + 1] = COSMIC_SPRITES[i].getV0();
				AvaritiaCosmicShaders.COSMIC_UVS[i * 4 + 2] = COSMIC_SPRITES[i].getU1();
				AvaritiaCosmicShaders.COSMIC_UVS[i * 4 + 3] = COSMIC_SPRITES[i].getV1();
			}
		}
	}
}
