...
public class ItemInfinityHoe extends HoeItem {
	...
	private boolean applyBonemeal(Level level, BlockPos pos, Player player) {
		...
		if (block instanceof BonemealableBlock && !(block instanceof GrassBlock) && !(block instanceof TallGrassBlock)
	...
	public InteractionResult useOn(UseOnContext context) {
		...
		if (player.isShiftKeyDown()) {
			if (!this.applyBonemeal(level, blockPos, player)) {
				BlockState blockState = level.getBlockState(blockPos).getToolModifiedState(context,
