...
public class AvaritiaRenderUtils {
		...
		for (int i = 0; i < sprites.length; ++i) {
			...
			for (BlockElement element : unbaked) {
				for (Entry<Direction, BlockElementFace> directionBlockElementFaceEntry : element.faces.entrySet()) {
					quads.add(FaceBakery.bakeQuad(element.from, element.to, directionBlockElementFaceEntry.getValue(), sprite, directionBlockElementFaceEntry.getKey(), new SimpleModelState(state), element.rotation, element.shade, 1));
				}
			}
		}
		return quads;
	...
	public static void renderSingularity(ItemStack stack, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay) {
		...
		for (int i = 0; i < 2; i++) {
			List<BakedQuad> quads = AvaritiaRenderUtils.bakeItem(new TextureAtlasSprite[] { textureAtlas.getSprite(ResourceLocation.tryBuild(Avaritia.MODID, "item/singularity/singularity_layer_" + i)) });
